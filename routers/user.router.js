...
router.post('/signup', async (req, res) => {
	try {
		...
		if (!emailCheck) {
			return res.status(401).json({ message: '유효한 이메일 형식이 아닙니다.' });
		}
		// 비밀번호 6자 이상 - 확인
		...
		return res
			.status(201)
			.send({ email, name, message: '회원가입에 성공하였습니다.' });
	} catch {
...
router.post('/login', async (req, res) => {
	try {
		...
		if (!email || !password) {
			return res
				.status(400)
				.json({ message: '아이디나 비밀번호 란이 비어있습니다.' });
		}
		//데이터베이스에 존재하지 않는 이메일
		if (!existUser) {
...
router.put('/myinfo', authMiddleware, async (req, res) => {
	try {
		...
		if (!email || !password || !newPassword) {
			return res
				.status(400)
				.json({ message: '비밀번호 변경을 위한 필수 정보가 누락되었습니다.' });
		}
		//비밀번호 hash 매치 - 기존비번
		const encryptPassword = encrypt(password);
		//새 비밀번호 hash
		...
		return res.status(400).json({ message: '비밀번호 변경에 실패하였습니다.' });
	}
});
...
router.delete('/myinfo', authMiddleware, async (req, res) => {
	try {
		...
		if (existUser.password === encryptPassword) {
			...
			return res.status(200).json({ message: '회원정보가 삭제되었습니다.' });
		} else {
			return res.status(401).json({ message: '비밀번호가 틀렸습니다.' });
		}
	} catch {
		return res.status(400).json({ message: '회원정보 삭제에 실패하였습니다.' });
	}
});
...
router.get('/myinfo', authMiddleware, async (req, res) => {
	try {
		...
		return res
			.status(200)
			.json({ '내 정보': { email, name: name, 가입일자: createdAt } });
	} catch {
		return res.status(400).json({ message: '내 정보 조회에 실패하였습니다.' });
	}
});
